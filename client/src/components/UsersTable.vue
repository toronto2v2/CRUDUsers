        ...
        <table class="table table-striped">
            <thead>
                <tr @click="onFiltersClick($event)">
                    ...
                    <th :data-filter="'closestEvent'" scope="col">
                        Closest Event
                    </th>
                    <th scope="col"></th>
                </tr>
            ...
            <tbody v-if="!filteredUsers" key="firstPlate">
                ...
                <tr
                    @click="onUserClick(user._id, $event, $router)"
                    v-for="(user, index) in users.users"
                    :key="user._id"
                    class="m-5 sectionHover"
                >
...
const filterByClosestEvent = (activeFilter, originalUsers) => {
    ...
    if (activeFilter) {
        filteredUsers.sort(
            (a, b) =>
                +a.closestEvent.toString().replace(/\D/g, "") -
                +b.closestEvent[0].toString().replace(/\D/g, "")
        );
    } else {
        filteredUsers.sort(
            (a, b) =>
                +b.closestEvent[0].toString().replace(/\D/g, "") -
                +a.closestEvent[0].toString().replace(/\D/g, "")
        );
    }
    return filteredUsers;
};
export default {
    ...
    methods: {
        ...
        onFiltersClick(e) {
            if (this.activeFilter !== e.target.dataset.filter)
                this.activeFilter = "";
            ...
            switch (e.target.dataset.filter) {
                case "phone":
                    this.filteredUsers = filterByPhone(
                        this.activeFilter,
                        this.users.users
                    );
                    break;
                case "eventCount":
                    this.filteredUsers = filterByEventCount(
                        this.activeFilter,
                        this.users.users
                    );
                    break;
                case "closestEvent":
                    this.filteredUsers = filterByClosestEvent(
                        this.activeFilter,
                        this.users.users
                    );
                    break;
                default:
                    this.filteredUsers = filterByLetters(
                        this.activeFilter,
                        this.users.users,
